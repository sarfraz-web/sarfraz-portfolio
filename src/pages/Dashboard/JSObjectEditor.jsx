...
const JSObjectEditor = () => {
  ...
  const repoFullName =
    import.meta.env.VITE_GITHUB_REPO_FULL_NAME || "sarfraz-web/sarfraz-portfolio";
  ...
  const getRepo = async (fileName) => {
    ...
    try {
      const res = await fetch(
        `https://api.github.com/repos/${repoFullName}/contents/frontend/src/data/${fileName}.js`,
        {
          method: "GET",
          headers: {
  ...
  // }
    ...
    const res = await fetch(
      `https://api.github.com/repos/${repoFullName}/contents/frontend/src/data/${currentFile}.js`,
      {
        method: "PUT",
        headers: {
          Authorization: import.meta.env.VITE_GITHUB_REPO_TOCKEN,
